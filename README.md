...
- `src/trayffeine/settings.py`: persisted settings storage for language selection, infinite restore, and detailed logging
...
- `src/trayffeine/windows.py`: Windows backend helpers for `SendInput`, message boxes, confirmation dialogs, and shell open
...
python scripts\generate_assets.py
powershell -ExecutionPolicy Bypass -File packaging\windows\build.ps1 -Version 0.6.2 -Clean
```
...
- tags matching `v*-beta*` are still published as GitHub prereleases, but `0.6.2` is a normal stable release
